...
from . import shield as shield_cmds
from ..game import weapons, customizations, shields
from ..game.helpers.shopabstract import ShopBuySellItem
from functools import wraps

### Fill in the blanks buy/sell functions
DEPARTMENT_NOT_FOUND = "Department not found"
ITEM_NOT_FOUND = "Item not found! You also can't buy/sell weapons with alias names, only their normal names!"
...
def shop_shields_list(page, **details):
    shop_shields = list(shields.get_shields_for_server(details["server_id"]).values())
    shop_shields.remove(shields.NO_SHIELD)
    if "None" in shop_shields:
        shop_shields.pop(shop_shields.index("None"))
    if "Test" in shop_shields:
        shop_shields.pop(shop_shields.index("Test"))
    if "Other Broken Shield" in shop_shields:
        shop_shields.pop(shop_shields.index("Other Broken Shield"))
    shop_shields.sort(key=lambda shield: shield.price)
    for shield in shop_shields:
        shield.price = shield._price()
        if shield.name == "None" or shield.name == "Other Broken Shield" or shield.name == "Test":
            shop_shields.remove(shield)
    shop_list = shield_cmds.shields_page(shop_shields, page, "DueUtil's Shield Shop!",
                                       footer_more=("But wait there's more! Do "
                                                    + details["cmd_key"] + "shop shields " + str(page + 2)),
                                       footer_end=('Want more? Ask an admin on '
                                                   + details["server_name"] + ' to add some!'))
    return shop_list
...
async def item_action(item_name, action, department=None, **details):
    ...
    if department is None:
        ...
        if len(possible_departments) > 1:
            ...
            await util.say(channel, ITEM_NOT_FOUND)
            return
...
departments = {
    ...
    "shields": {
        ...
        "actions": {
            "info_action": shield_cmds.shield_info,
            "list_action": shop_shields_list,
            "buy_action": shield_cmds.buy_shield,
            "sell_action": shield_cmds.sell_shield
        },
        "item_exists": lambda details, name: name.lower() != "none" and shields.does_shield_exist(details["server_id"],
                                                                                                  name),
        "item_exists_sell": lambda details, name: ((name != "none" and details["author"].shield.name.lower() == name
                                                   or details["author"].owns_shield(name))) if hasattr(details["author"], "shield") else lambda details, name: ((name != "none" and details["author"].owns_shield(name)))
...
#            if args[0] == "shields" or args[0] == "shield":
