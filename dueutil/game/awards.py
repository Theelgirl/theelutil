...
async def take_award(channel, player, award_id, text=None):
    if get_award(award_id) is not None and award_id in player.awards:
        player.awards.remove(award_id)
        player.save()
        if not channel.is_private and dueserverconfig.mute_level(channel) < 0:
            await util.say(channel, "Done!")
            update_award_stat(award_id, "times_given", -1)
