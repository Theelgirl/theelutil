...
class Quest(DueUtilObject, SlotPickleMixin):
    ...
    def __init__(self, name, base_attack, base_strg, base_accy, base_hp, **extras):
        ...
        self.times_beaten = 0
        self.rank = extras.get('rank', 1000)
        self._add()
        self.save()
...
class ActiveQuest(Player, util.SlotPickleMixin):
    ...
    async def create(q_id: str, quester: Player):
        ...
        target_exp = random.uniform(quester.total_exp, quester.total_exp*1.5)
    ...
    def get_reward(self):
        ...
        return max(1, int(base_reward + base_reward * (self.get_quest_scale() + 1) * 10 / 3))
