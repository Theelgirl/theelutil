...
class Weapon(DueUtilObject, SlotPickleMixin):
    ...
    __slots__ = ["damage", "accy", "price",
                 ...
                 "weapon_sum", "server_id", "upgrades", "alias"]
    ...
    def __init__(self, name, hit_message, damage, accy, **extras):
        ...
        if message is not None:
            ...
            alias = extras.get('alias', name)
            if not Weapon.acceptable_string(alias, 30):
                raise util.DueUtilException(message.channel, "Weapon aliases must be between 1 and 30 characters! If not specified when created, weapon aliases are the name.")
        ...
        self.alias = extras.get('alias', name)
