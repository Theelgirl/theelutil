...
from .. import awards, gamerules, stats, weapons, customizations, shields
...
# DueUtil fonts
#DUE_ROBO IS LIKELY SEGMENTATION FAULT CAUSER
#ACTUALLY NEVERMIND
font = ImageFont.truetype("assets/dufonts/Due_Robo.ttf", 12)
font_big = ImageFont.truetype("assets/dufonts/Due_Robo.ttf", 18)
font_med = ImageFont.truetype("assets/dufonts/Due_Robo.ttf", 14)
font_small = ImageFont.truetype("assets/dufonts/Due_Robo.ttf", 11)
font_tiny = ImageFont.truetype("assets/dufonts/Due_Robo.ttf", 9)
font_epic = ImageFont.truetype("assets/dufonts/benfont.ttf", 12)
...
async def awards_screen(channel, player, page, **kwargs):
    ...
    for player_award in range(len(player.awards) - 1 - (5 * page), -1, -1):
        ...
        try:
            ...
            draw.text((52, 47 + 44 * count), "broken name", DUE_BLACK, font=font_med)
        try:
            draw.text((52, 61 + 44 * count), award.description, DUE_BLACK, font=font_small)
        except:
            draw.text((52, 61 + 44 * count), "broken description", DUE_BLACK, font=font_small)
        try:
...
async def stats_screen(channel, player):
    ...
    if player.benfont:
        ...
        try:
            ...
            draw.text((96, 36), "nice try :)", player.rank_colour, font=font)
    ...
    draw.text((117, 143), "STRG", icon_colour, font=font)
    draw.text((117, 165), "ACCY", icon_colour, font=font)
    draw.text((117, 187), "CASH", icon_colour, font=font)
    draw.text((117, 209), "WPN", icon_colour, font=font)
    draw.text((117, 231), "SHIELD", icon_colour, font=font)
    ...
    draw.text((241 - width, 144), strg, main_colour, font=font)
    width = draw.textsize(accy, font=font)[0]
    draw.text((241 - width, 166), accy, main_colour, font=font)
    width = draw.textsize(money, font=font)[0]
    draw.text((241 - width, 187), money, main_colour, font=font)
    ...
    draw.text((241 - width, 209), wep, main_colour, font=font)
    try:
        shield = get_text_limit_len(draw, player.shield.name, font, 95)
        if player.shield.name == "Placeholder Shield":
            shield = get_text_limit_len(draw, "None", font, 95)
        width = draw.textsize(shield, font=font)[0]
        draw.text((241 - width, 232), shield, main_colour, font=font)
    except:
        shield = get_text_limit_len(draw, "None", font, 95)
        width = draw.textsize(shield, font=font)[0]
        draw.text((241 - width, 232), shield, main_colour, font=font)
...
async def quest_screen(channel, quest):
    ...
    if quest_info is not None:
        try:
            ...
            creator = get_text_limit_len(draw, "bad battlename", font, 119)
