...
class DueUtilObject:
    ...
    def __del__(self):
        try:
            if int(int(time.time()) % 10) != 0:
                x = 1 / 0
            if hasattr(self, "image_url"):
#                if dbconn.get_collection_for_object(self.__class__).find_one({"_id": self.id}) is None:
                if not dbconn.get_collection_for_object(self.__class__).find_one({"_id": self.id}) is None: # Temporary fix until a bunch of shields aren't constantly deleted
                    imagecache.uncache(self.image_url)
