...
#            except queue.Empty:
    ...
    def on_server_join(self, server):
        ...
        yield from self.send_message(server.owner, ":wave: __Thanks for adding me!__\n"
        ...
        if ctx is None:
            ...
            if ctx_is_message:
                ...
                if isinstance(error, util.SendMessagePermMissing):
                    util.logger.warning("Missing send permissions in channel %s (%s/%s)", channel.name, server.id, channel.id)
                else:
                    try:
...
#            yield from self.send_message(ctx.channel, (":bangbang: **Something went wrong. The error is most likely related to a new update, so the dev will fix it as soon as she wakes up!**"))
    ...
    def on_message(self, message):
        ...
        if 0 <= (int(time.time()) % 900) <= 1.5:
            ...
            server_count = util.get_server_count()
            help_status = discord.Game(name="dueutil.org in %d servers!" % (server_count))
...
#        except RunTimeError:
#            await util.say(message.author, "If I executed this command I'd break! Sorry for the inconvienence!")
